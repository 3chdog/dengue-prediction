...
# 定義日期區間
start_date = datetime(2005, 1, 1)  # 開始日期
...
def run(playwright):
    browser = playwright.chromium.launch(headless=True)  # 啟動瀏覽器
    print('browser open')
...
app_ui = ui.page_fixed(
    ...
    ui.div(
        ui.markdown(
            """# 新版中央氣象局觀測資料查詢系統（CODiS）爬蟲程式     
            這是一個用於下載中央氣象局觀測資料查詢系統（CODiS）的爬蟲程式。
            中央氣象局[觀測資料查詢系統（CODiS）](https://codis.cwa.gov.tw/StationData)
            """
        ),
        class_="mb-5",
    ),
    ui.row(
        ui.column(
            ...
            ui.output_text_verbatim("counter"),
        ...
        ui.column(
            ...
            ui.panel_well(
                ui.input_text(
                    id="base_download_path",
                    label="下載路徑：",
                    value=base_download_path
                ),
                class_="mb-3",
            ),
            ui.panel_well(
                ui.input_action_button("action_button", "下載資料"),
                # ui.button(
                #     id="action_button",
                #     label="下載資料",
...
def  server(input: Inputs, output: Outputs, session: Session):
    @session.on("click", "download_button")
    ...
    def download_data():
        ...
        base_download_path = input.base_download_path

        # 下載資料
        download_data(station_name, station_code, start_date, end_date, base_download_path)
    def plot():
        df = load_penguins()
        mass = df["body_mass_g"]
        fig, ax = plt.subplots()
        ax.hist(mass, input.action_button(), density=True)
    ...
    @reactive.event(input.action_button)
    def counter():
        return f"{input.action_button()} times clicked"
